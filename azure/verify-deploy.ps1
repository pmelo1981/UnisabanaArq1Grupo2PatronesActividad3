...
Write-Host "║    ✔️  VERIFICAR DEPLOYMENT                  ║" -ForegroundColor Cyan
Write-Host "╚═══════════════════════════════════════════════╝`n" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "1️⃣  Verificando namespace '$Namespace'..." -ForegroundColor Yellow
    ...
    Write-Host "2️⃣  Verificando Deployment..." -ForegroundColor Yellow
    ...
    if ($deployment) {
        ...
        if ($ready -eq $desired -and $ready -gt 0) {
            ...
            Write-Host "⚠️  Esperando pods... ($ready/$desired)`n" -ForegroundColor Yellow
        }
    } else {
    ...
    # 3. Verificar Pods
    Write-Host "3️⃣  Verificando Pods..." -ForegroundColor Yellow
    ...
    if ($pods) {
        ...
        if ($runningPods -gt 0) {
            ...
            Write-Host "⚠️  Pods no están Running aún`n" -ForegroundColor Yellow
        }
    } else {
    ...
    # 4. Verificar HPA
    Write-Host "4️⃣  Verificando Horizontal Pod Autoscaler..." -ForegroundColor Yellow
    $hpa = kubectl get hpa -n $Namespace -l app=productapi 2>$null
    if ($hpa) {
        $hpaName = kubectl get hpa -n $Namespace -l app=productapi -o jsonpath='{.items[0].metadata.name}' 2>$null
        ...
        if ($minReplicas -eq 2 -and $maxReplicas -eq 5) {
            ...
            Write-Host "⚠️  HPA valores inesperados`n" -ForegroundColor Yellow
        }
    } else {
        Write-Host "⚠️  HPA no encontrado (puede no estar habilitado)`n" -ForegroundColor Yellow
    }

    # 5. Verificar Service
    Write-Host "5️⃣  Verificando Service..." -ForegroundColor Yellow
    ...
    if ($svc) {
        ...
        if ($svcType -eq "ClusterIP") {
            Write-Host "✅ Service ClusterIP (acceso vía NGINX Ingress)`n" -ForegroundColor Green
        } else {
            $svcIP = kubectl get svc -n $Namespace $svc -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>$null
            Write-Host "   IP: $(if ($svcIP) { $svcIP } else { '<PENDING>' })"
            if ($svcIP) {
                Write-Host "✅ Service tiene IP`n" -ForegroundColor Green
            } else {
                Write-Host "⚠️  IP pendiente (puede tardar minutos)`n" -ForegroundColor Yellow
            }
        }
    } else {
    ...
    # 6. Verificar Ingress
    Write-Host "6️⃣  Verificando Ingress..." -ForegroundColor Yellow
    ...
    if ($ing) {
        ...
        if ($ingIP) {
            ...
            Write-Host "⚠️  IP NGINX pendiente`n" -ForegroundColor Yellow
        }
    } else {
        Write-Host "⚠️  Ingress no encontrado (acceso directo deshabilitado)`n" -ForegroundColor Yellow
        $ingIP = $null
    }

    # 7. Verificar Health Endpoint
    Write-Host "7️⃣  Probando Health Endpoint..." -ForegroundColor Yellow
    if ($ingIP) {
        try {
            ...
            if ($response -eq "200") {
                ...
                Write-Host "⚠️  Endpoint responde con $response (esperando 200)`n" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "⚠️  No se puede alcanzar el endpoint (puede estar iniciando)`n" -ForegroundColor Yellow
        }
    } else {
    ...
    Write-Host "8️⃣  Verificando ArgoCD Application..." -ForegroundColor Yellow
    ...
    if ($app) {
        ...
        if ($syncStatus -eq "Synced") {
            ...
            Write-Host "⚠️  Application no sincronizada`n" -ForegroundColor Yellow
        }
    } else {
        Write-Host "⚠️  ArgoCD Application no encontrada (verifica setup-argocd.ps1)`n" -ForegroundColor Yellow
    }

    # 9. Resumen
    Write-Host "╔═══════════════════════════════════════════════╗" -ForegroundColor $(if ($allOK) { "Green" } else { "Yellow" })
    Write-Host "║  $(if ($allOK) { '✅ VERIFICACIÓN COMPLETADA' } else { '⚠️  PARCIALMENTE VERIFICADO' })           ║" -ForegroundColor $(if ($allOK) { "Green" } else { "Yellow" })
